...
pnpm prisma:push      # 同步架构到数据库 (无迁移文件，开发用)
...
### 文件结构与命名
- **路由页面**: `app/pages/index.vue` (Nuxt 4 使用 `app/` 而非根 `pages/`)
...
- **测试文件**: 与被测文件同级或 `__tests__` 目录，后缀 `.test.ts` 或 `.spec.ts`
...
// ❌ 严禁 any (prisma/seed.ts:40, 266 需修复)
const data: any = { /* ... */ }
```
...
// ❌ 严禁创建多个实例
const client1 = new PrismaClient() // 错误
```
...
try {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    if (error.code === 'P2002') {
      throw createError({ statusCode: 409, statusMessage: 'Word already exists' })
    }
  }
  throw error
}
...
6. 每个错误场景都应有对应测试
7. 使用 beforeEach/afterEach 隔离测试数据
...
- ❌ 测试实现细节而非行为
- ❌ 在测试中使用生产数据库
- ❌ 重复代码 (如 seed.ts 和 reset.ts 的数据库清理逻辑)

## 项目独特风格

- **Prisma Client 输出**: 自定义到 `app/generated/prisma/` (非 node_modules)
...
| 环境变量 | `.env` (DATABASE_URL) |
| 路由页面 | `app/pages/` (需创建) |
| API 端点 | `server/api/` (需创建) |
...
- 使用 pnpm (pnpm-workspace.yaml 配置)
